  ...
  return (
    <div className="Paginations">
      <div className="Paginations_Show">
        <span>
          {" "}
          SHOWING {(current - 1) * 10 + 1}-{(current - 1) * 10 + totalRecords}{" "}
          of {total_records}{" "}
        </span>
      </div>
      <div className="Paginations_Pages ">
        {total_pages > 1 && current + 1 >= total_pages && (
          <FiChevronLeft onClick={() => setPage(current - 1)} />
        )}
        {current > 4 ? (
          <ul className="flex">
            <li>1</li>
          ...
          <ul className="flex">
            {Array.from(Array(total_pages).keys()).map((num) => (
              <li
                key={num}
                onClick={() => setPage(num + 1)}
                className={current - 1 === num ? "active" : ""}
              >
                {num + 1}
              </li>
            ))}
        ...
        {total_pages > 1 && current + 1 <= total_pages && (
          <FiChevronRight onClick={() => setPage(current + 1)} />
        )}
      </div>
    </div>
  );
