...
import {
  TagerInitState,
  TagerSchema,
} from "../Form/Validations/TagerValidation";
function AddTager({ AnimateAddTager, HandleAddTagerAnimate }: PropsType) {
  ...
  return (
    <div className="AddTager">
      <div
        className={`content  ${AnimateAddTager && "animate_content"}`}
              ...
              <Field name="name">
                {({ field, meta }: any) => (
                  <Input
                    ...
                    onChange={(e) => {
                      setTagerData({ ...TagerData, name: e.target.value });
              ...
              <Field name="phone">
                {({ field, meta }: any) => (
                  <Input
                    ...
                    onChange={(e) => {
                      setTagerData({ ...TagerData, phone: e.target.value });
                      console.log(e.target.value);
                    }}
                    className="w-full md:w-2/3"
                    field={field}
