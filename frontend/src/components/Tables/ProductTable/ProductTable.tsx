...
function ProductTable({ data }: ProductProps) {
  ...
  return (
    <>
      ...
      {DeleteModel && (
        <Model
          title={`Are You Sure! Want to Delete  ${SelectedProduct?.title} ?`}
                ...
                <td className="flex items-center flex-wrap md:flex-nowrap">
                  {itm.mainImage && (
                    <img
                      src={itm.mainImage}
                      alt={itm.title}
                      className="object-cover rounded-full border border-gray-300 mr-2 w-[40px] h-[40px]"
                    />
                  )}
                  {itm.title}
                </td>
