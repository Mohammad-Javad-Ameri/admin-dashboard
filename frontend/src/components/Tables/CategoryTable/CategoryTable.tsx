...
function CategoryTable({ data }: CategoryTableProp) {
  ...
  const [selectedCategory, setSelectedCategory] =
    useState<CategoryProps | null>(null); // to edit clicked category
  ...
  return (
    <>
      <div className="GeneralTable">
        ...
        {DeleteModel && (
          <Model
            title={`Are You Sure! Want to Delete ${selectedCategory?.title} ?`}
            description="Lorem ipsum dolor sit amet consectetur adipisicing elit. Facere quod sequi cumque aut deleniti cum consectetur expli"
            icon={<FiTrash2 size={24} color="red" />}
            actionFun={DeleteCategory}
            ...
            {data.categories.map((itm, index) => (
              <tr>
                ...
                <td className="flex items-center">
                  {itm.mainImage && (
                    <img
                      src={itm.mainImage}
                      alt={itm.title}
                      className="object-cover rounded-full border-2 border-gray-300 mr-2 w-[40px] h-[40px] "
                    />
                  )}
                </td>
                <td>{itm.title}</td>
                ...
                <td>
                  <SwitchBtn
                    id={itm._id}
                    ChangeStataus={ChangeStataus}
                    status={itm.published ? "done" : "notyet"}
                  />
                </td>
                <td className="flex">
                  <FiEdit
        ...
        {!data.categories.length && (
          <div className="flex justify-center items-center flex-col mx-auto py-3 my-3">
            ...
            <h1 className=" text-gray-600 capitalize  text-2xl pt-5">
              Sorry, we can not find this Category 😞
            </h1>
          </div>
        )}
      </div>
